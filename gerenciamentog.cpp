...
void Gerenciamento::addLivros()     //add "n" livros na biblioteca
{
	int n;
	...
	cout << "Digite quantos livros deseja registrar: ";
	cin >> n;
	
	for(int i=0;i<n;i++)
	{
		Livros book;	// cria um objeto da classe Livros
		book.inicializa();
		livros.push_back(book);
	}
	
	ofstream meuarquivo3(arquivo);
	if (meuarquivo3.is_open())
	{
		for(it=livros.begin();it!=livros.end();++it)
		{
			meuarquivo3 <<  (*it).titulo << endl;
			meuarquivo3 <<  (*it).autor << endl;
			meuarquivo3 <<  (*it).editora << endl;
			meuarquivo3 <<  (*it).edicao << endl; 
			meuarquivo3 <<  (*it).nExemplares << endl;
			meuarquivo3 <<  (*it).codigoIndividuosSolicitantes << endl;
		}
...
{	
	...
	cout << "insira seu código de usuario.\n";
	cin >> code;
	cout << endl;
	
	for(it=codigos_individuos.begin();it!=codigos_individuos.end();++it)
	{
		if(code== *it)
		{
			cout << "Cliente encontrado!\n";
			cout << endl;
			cout << "Qual livro deseja locar?\n";
			cin >> nome;
			
			for(li=livros.begin();li!=livros.end();++li)
			{
				if(nome == (*li).titulo)
				{	
					if((*li).nExemplares==0)
					{
						cout << "Esse livro não está mais disponivel.\n";
					}
					else
					{
						(*li).codigoIndividuosSolicitantes= (*li).condigoIndividuosSolicitantes +", "+to_string(code)
						(*li).nExemplares--;
					}
				}
				else
